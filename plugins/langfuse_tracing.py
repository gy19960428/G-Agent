...
- g_agent.loop.agent_runner_loop        -> outer agent trace (parent of all below)
- g_agent.llm._write_llm_log              -> generation span (Prompt=start, Response=end)
...
try:
    from g_agent.llm import _load_mykeys
...
if _lf:
    from g_agent import llm, loop

    _tls = threading.local()

    _orig_log = llm._write_llm_log
    ...
    llm._write_llm_log = _patched_log
    ...
    llm._parse_claude_sse = _wrap_parser(llm._parse_claude_sse)
    llm._parse_openai_sse = _wrap_parser(llm._parse_openai_sse)

    _orig_before = loop.BaseHandler.tool_before_callback
    _orig_after = loop.BaseHandler.tool_after_callback
    ...
    def _patched_after(self, tool_name, args, response, ret):
        try:
            if getattr(_tls, "tstack", None):
                sp = _tls.tstack.pop()
                out = (
                    {"data": ret.data, "next_prompt": ret.next_prompt, "should_exit": ret.should_exit} if ret else None
                )
    ...
    loop.BaseHandler.tool_before_callback = _patched_before
    loop.BaseHandler.tool_after_callback = _patched_after

    _orig_loop = loop.agent_runner_loop

    def _patched_loop(client, system_prompt, user_input, handler, tools_schema, *a, **kw):
        try:
            cm = _lf.start_as_current_observation(name="agent.task", as_type="agent", input={"user_input": user_input})
        except Exception:
            cm = None
        if cm is None:
    ...
    loop.agent_runner_loop = _patched_loop
