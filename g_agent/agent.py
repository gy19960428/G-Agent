...
os.environ.setdefault('G_AGENT_LANG', 'zh' if any(k in (locale.getlocale()[0] or '').lower() for k in ('zh', 'chinese')) else 'en')
...
class Agent:
    def __init__(self):
        ...
        try:
            ...
            if not os.path.exists(_side):
                with open(_side, 'w', encoding='utf-8') as _f: _f.write(_ch)
    ...
    def run(self):
        while True:
            ...
            _t_task = time.time(); _q_preview = raw_query[:80].replace('\n', ' ') if raw_query else ''
            print(f"[AGENT] task.begin source={source} qlen={len(raw_query or '')} qsize={self.task_queue.qsize()} preview={_q_preview!r}", flush=True)
            raw_query = self._handle_slash_cmd(raw_query, display_queue)
            if raw_query is None:
                print(f"[AGENT] task.end source={source} elapsed={time.time()-_t_task:.1f}s result=slash_cmd", flush=True)
            ...
            if source == 'feishu' and not raw_query.startswith('/') and len(self.history) > 1:   # 飞书普通消息才注入锚点，避免命令链路被包装
            ...
            try:
                full_resp = ""; last_pos = 0; last_progress_display = ""; completed_turns = set(); last_turn = None
                for chunk in gen:
                    ...
                    if source == 'feishu':
                        ...
                        seen_turns = {
                            item['turn'] for item in _feishu_turn_summaries(full_resp)
                        ...
                        if cur_turn is not None and cur_turn != last_turn:
                            events.append(_feishu_event('turn_start', turn=cur_turn, text=full_resp, display=cur_display or f'Turn {cur_turn}：思考中', status='running'))
                        ...
                        if events:
                            display_queue.put({'next': full_resp, 'progress': progress, 'turn_updates': [], 'events': events, 'source': source})
                            if cur_display:
                ...
                print(f"[AGENT] task.end source={source} elapsed={time.time()-_t_task:.1f}s turns={_turns} result={_result} resp_bytes={_resp_bytes}", flush=True)
...
if __name__ == '__main__':
    ...
    if args.task:
        ...
        if sys.stdout.isatty():
            ...
            try:
                sys.stdout.write(f'\x1b[92m✦\x1b[0m \x1b[1mG-Agent\x1b[0m '
