...
def agent_runner_loop(client, system_prompt, user_input, handler, tools_schema, 
    ...
    while turn < handler.max_turns:
        ...
        if (not _plan_reminded) and turn >= max(2, handler.max_turns // 2) \
        ...
        print(f"[LOOP] turn={turn} llm.done elapsed={time.time()-_t_llm:.1f}s tool_calls={_tc_cnt} content_bytes={_cb}", flush=True)
        ...
        for ii, tc in enumerate(tool_calls):
            ...
            print(f"[LOOP] turn={turn} tool.done name={tool_name} idx={ii} elapsed={time.time()-_t_tool:.1f}s exit={outcome.should_exit} has_next={bool(outcome.next_prompt)}", flush=True)
