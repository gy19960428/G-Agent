...
function createEnhancedDOMCopy() {  
  ...
  function cloneNode(sourceNode, keep=false) {  
    ...
    try { if (sourceNode.matches && sourceNode.matches(':-webkit-autofill')) { clone.setAttribute('data-autofilled', 'true'); if (!sourceNode.value) clone.setAttribute('value', '⚠️受保护-读browser_driver_sop的autofill章节提取'); } } catch(e) {}
...
const _fc = [...domCopy.querySelectorAll('*')].filter(el => {
  ...
  .slice(0, 2);
_fc.forEach(el => { const r = getNodeInfo(el).rect; console.log('[html_simplify] Hoisted fixed dialog:', el.tagName + (el.id ? '#'+el.id : '') + (el.className ? '.'+String(el.className).split(' ')[0] : ''), Math.round(r.width)+'x'+Math.round(r.height), Math.round(100*r.width*r.height/viewportArea)+'%'); el.parentNode.removeChild(el); domCopy.appendChild(el); });
