...
def __getattr__(name):  # once guard in PEP 562
    ...
    raise AttributeError(f"module 'g_agent.llm' has no attribute {name}")
...
_OAI_RESP_SSE_KNOWN_EVENTS = frozenset(
    {
        ...
        "error",
        "response.created",
        "response.in_progress",  # noisy lifecycle, intentionally ignored
...
def _warn_unknown_sse_event(scope, evt_type):
    ...
    key = (scope, evt_type)
    if key in _SEEN_UNKNOWN_SSE_EVENTS:
        return
            ...
            if current_block:
                if current_block["type"] == "tool_use":
                    try:
                        ...
                        current_block["input"] = {"_parse_error": f"sse json: {e}", "_raw": tool_json_buf}
    ...
    if current_block:
        if current_block["type"] == "tool_use":
            try:
                ...
                current_block["input"] = {"_parse_error": f"sse json (tail): {e}", "_raw": tool_json_buf}
        content_blocks.append(current_block)
        current_block = None
    if warn:
    ...
    if len(parts) > 1:
        ...
        for p in parts:
            try:
                ...
                return [{"_parse_error": f"split-json failed: {e}", "_raw": raw}]
        return parsed
    ...
    if api_mode == "responses":
        ...
        for item in data.get("output") or []:
            if item.get("type") == "message":
                ...
                try:
                    ...
                    args = {"_parse_error": f"function_call args: {e}", "_raw": item.get("arguments", "")}
                blocks.append(
                    {
                        "type": "tool_use",
                        "id": item.get("call_id", item.get("id", "")),
        ...
        for tc in msg.get("tool_calls") or []:
            ...
            try:
                ...
                args = {"_parse_error": f"chat tool_call args: {e}", "_raw": fn.get("arguments", "")}
...
def _stream_with_retry(sess, url, headers, payload, parse_fn):
    ...
    _model = getattr(sess, "model", "?")
    try:
        _payload_bytes = len(json.dumps(payload, ensure_ascii=False))
    except Exception:
        _payload_bytes = -1
    _msgs = payload.get("messages") if isinstance(payload, dict) else None
    _msg_cnt = len(_msgs) if isinstance(_msgs, list) else -1
    print(
        f"[LLM] begin model={_model} msgs={_msg_cnt} payload_bytes={_payload_bytes} max_retries={sess.max_retries} stream={getattr(sess,'stream',True)}",
        flush=True,
    )
    _t_begin = time.time()
        ...
        try:
            with requests.post(
                ...
                if r.status_code >= 400:
                    ...
                    print(
                        f"[LLM] fatal model={_model} status={r.status_code} attempt={attempt+1} elapsed={time.time()-_t_begin:.1f}s body={body[:200]!r}",
                        flush=True,
                    )
                    yield err
                ...
                try:
                    while True:
                        ...
                        if isinstance(_chunk, str):
                            _streamed_bytes += len(_chunk)
                        yield _chunk
                except StopIteration as e:
                    if not e.value and not streamed:
                        raise requests.ConnectionError("empty response")
                    print(
                        f"[LLM] done model={_model} status={r.status_code} attempt={attempt+1} elapsed={time.time()-_t_begin:.1f}s streamed_bytes={_streamed_bytes}",
                        flush=True,
                    )
                    return e.value or []
            ...
            if attempt < sess.max_retries:
                ...
                continue
            print(
                f"[LLM] fatal model={_model} exc={type(e).__name__} attempt={attempt+1} elapsed={time.time()-_t_begin:.1f}s",
                flush=True,
            )
            yield err
            return [{"type": "text", "text": err}]
            ...
            print(
                f"[LLM] fatal model={_model} exc={type(e).__name__}: {e!s:.200} attempt={attempt+1} streamed={streamed} elapsed={time.time()-_t_begin:.1f}s",
                flush=True,
            )
            yield err
            return [{"type": "text", "text": err}]
...
class ToolClient:
    ...
    def _prepare_tool_instruction(self, tools):
        ...
        _en = os.environ.get("G_AGENT_LANG") == "en"
        if _en:
            tool_instruction = """
...
class NativeToolClient:
    ...
    def _thinking_prompt():
        return THINKING_PROMPT_EN if os.environ.get("G_AGENT_LANG") == "en" else THINKING_PROMPT_ZH
