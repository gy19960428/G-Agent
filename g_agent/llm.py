...
def _stream_with_retry(sess, url, headers, payload, parse_fn):
    ...
    try: _payload_bytes = len(json.dumps(payload, ensure_ascii=False))
    except Exception: _payload_bytes = -1
    _msgs = payload.get('messages') if isinstance(payload, dict) else None
    _msg_cnt = len(_msgs) if isinstance(_msgs, list) else -1
    print(f"[LLM] begin model={_model} msgs={_msg_cnt} payload_bytes={_payload_bytes} max_retries={sess.max_retries} stream={getattr(sess,'stream',True)}", flush=True)
    _t_begin = time.time()
        ...
        try:
            with requests.post(url, headers=headers, json=payload, stream=sess.stream, 
                ...
                if r.status_code >= 400:
                    ...
                    print(f"[LLM] fatal model={_model} status={r.status_code} attempt={attempt+1} elapsed={time.time()-_t_begin:.1f}s body={body[:200]!r}", flush=True)
                    yield err; return [{"type": "text", "text": err}]
                gen = parse_fn(r)
                try:
                    while True:
                        _chunk = next(gen); streamed = True
                        if isinstance(_chunk, str): _streamed_bytes += len(_chunk)
                        yield _chunk
                except StopIteration as e:
                    if not e.value and not streamed: raise requests.ConnectionError("empty response")
                    print(f"[LLM] done model={_model} status={r.status_code} attempt={attempt+1} elapsed={time.time()-_t_begin:.1f}s streamed_bytes={_streamed_bytes}", flush=True)
                    return e.value or []
            ...
            print(f"[LLM] fatal model={_model} exc={type(e).__name__} attempt={attempt+1} elapsed={time.time()-_t_begin:.1f}s", flush=True)
            yield err; return [{"type": "text", "text": err}]
        except Exception as e:
            err = f"\n\n[!!! 流异常中断 {type(e).__name__}: {e} !!!]" if streamed else f"!!!Error: {type(e).__name__}: {e}"
            print(f"[LLM] fatal model={_model} exc={type(e).__name__}: {e!s:.200} attempt={attempt+1} streamed={streamed} elapsed={time.time()-_t_begin:.1f}s", flush=True)
            yield err; return [{"type": "text", "text": err}]
...
class NativeToolClient:
    @staticmethod
    def _thinking_prompt(): return THINKING_PROMPT_EN if os.environ.get('G_AGENT_LANG') == 'en' else THINKING_PROMPT_ZH
