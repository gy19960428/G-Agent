...
from g_agent.loop import BaseHandler, StepOutcome, json_default
...
class ToolHandler(BaseHandler):
    ...
    def do_ask_user(self, args, response):
        ...
        result = ask_user(question, candidates, multi=multi)
    ...
    def do_file_write(self, args, response):
        ...
        try:
            ...
            if self._maybe_auto_enter_plan(path):
                yield f"[Info] 自动进入 plan 模式 (max_turns→100)：{path}\n"
            next_prompt = self._get_anchor_prompt(skip=args.get('_index', 0) > 0)
    ...
    def _maybe_auto_enter_plan(self, path):
        # 写入路径形如 .../plan_XXX/plan.md 时，若未挂挡则自动进入 plan 模式（兜底 plan_sop 漏挡）
        if self._in_plan_mode(): return False
        if re.search(r'(^|/)plan_[^/]+/plan\.md$', (path or '').replace('\\', '/')):
            self.enter_plan_mode(path); return True
        return False
...
''' + get_global_memory()
    ...
    def turn_end_callback(self, response, tool_calls, tool_results, turn, next_prompt, exit_reason):
        summary = extract_turn_brief(response.content)
        if not summary:
        ...
        for hook in list(getattr(self.parent, '_turn_end_hooks', {}).values()): hook(locals())  # snapshot to avoid dict-changed-during-iteration
        return next_prompt

def get_global_memory():
    prompt = "\n"
    try:
        suffix = '_en' if os.environ.get('G_AGENT_LANG', '') == 'en' else ''
