...
def check():
    ...
    for f in sorted(os.listdir(TASKS)):
        ...
        if (now_minutes - sched_minutes) > max_delay * 60:
            _logger.info(
                f"SKIP {tid}: {now_minutes - sched_minutes}min past schedule, " f"exceeds max_delay={max_delay}h"
            )
            continue
        ...
        # 触发
        _logger.info(f"TRIGGER {tid} (repeat={repeat}, schedule={sched}, " f"last_run={last})")
