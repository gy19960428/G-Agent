# ══════════════════════════════════════════════════════════════════════════════
#  G-Agent — mykey.py 配置模板（复制为 mykey.py 后填入真实凭证）
# ══════════════════════════════════════════════════════════════════════════════
#
#  ┌─────────────────────────────────────────────────────────────────────────┐
#  │ 快速上手：只需 3 步                                                      │
...
#  │  3. 运行 python -m g_agent.agent 或 g-agent fs / wechat / tg                │
#  └─────────────────────────────────────────────────────────────────────────┘
#
#  ────────── Session 类型速查 ──────────
#
#  g_agent/agent.py 只扫描变量名同时包含 'api' / 'config' / 'cookie' 的条目，
...
#     #          none/minimal 被 g_agent.llm 打 WARN 丢弃（Claude 不支持这两档）
...
#  注意：Kimi/Moonshot 温度会被 g_agent/llm.py 强制改为 1.0，写什么都会被覆盖。
# oai_config_kimi = {
...
#     # 'temperature': 0.3,                          # ← 无效，会被 g_agent.llm 覆盖为 1.0
#     # 'max_tokens': 8192,                          # int 默认 8192
# }
