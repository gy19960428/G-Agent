...
1. 打开 [GitHub 仓库页面](https://github.com/lsdefine/G-Agent)
...
> 💡 Windows 上如果 `python3` 不识别，换成 `python -m g_agent.agent`。
...
依赖装完后，用统一的 `g-agent` CLI 启动你的 Bot 前端：

```bash
g-agent fs        # 飞书 / Lark
g-agent wechat    # 个人微信（iLink 协议）
g-agent tg        # Telegram
```

也可以用 `g-agent cli` 在终端直接对话，或 `g-agent hub` 打开管理面板。
...
> 💡 这就是 G-Agent 的核心设计理念：**代码即文档**。Agent 能读懂自己的源码，所以任何功能你都可以直接问它。

---

## 💡 使用越久越强

G-Agent 不预设技能，而是**靠使用进化**。每完成一个新任务，它会自动将执行路径固化为 Skill，下次遇到类似任务直接调用。
