"""Per-thread LLM token usage via g_agent.llm monkey-patches.

`install()` wraps `g_agent.llm._record_usage` + `g_agent.llm.print` (the SSE
...
# G-Agent's real context budget lives on `BaseSession.context_win` (chars). The
# trim trigger is `context_win * 3` (see g_agent.llm.trim_messages_history), so
...
def install() -> None:
    """Idempotently wrap llm._record_usage and llm.print."""
    global _INSTALLED
    if _INSTALLED:
        return
    from g_agent import llm

    orig_record, orig_print = llm._record_usage, print
    ...
    llm._record_usage = record_patched
