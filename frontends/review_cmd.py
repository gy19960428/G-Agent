...
def _render_prompt(user_request: str) -> str:
    """加载 /review inline prompt 并注入 user_request + code_root。"""
    lang = os.environ.get("G_AGENT_LANG", "").strip().lower()
    ...
    try:
        with open(fpath, "r", encoding="utf-8") as f:
            return f.read().format(user_request=user_request, code_root=code_root)
...
def _help_text() -> str:
    return (
        ...
        "例:\n"
        "  `/review`\n"
        "  `/review 我刚改了 review_cmd.py 和 fsapp.py,关注 prompt 注入`\n"
...
def handle(agent, body: str, display_queue) -> Optional[str]:
    ...
    if body in ("help", "?", "-h", "--help"):
        ...
        return None
    en = os.environ.get("G_AGENT_LANG", "").strip().lower() == "en"
