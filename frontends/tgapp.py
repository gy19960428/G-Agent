...
_ASK_DONE_ACTION = "done"
_ASK_OWNER = "telegram_singleton"
_BUS = AskUserEventBus()
# multi-select 启发式：问题文本含 [多选] / [multi] 即按多选渲染
_MULTI_SELECT_RE = re.compile(r"\[多选\]|\[multi\]", re.I)
# tg 侧菜单状态（与 _BUS 双写，callback 端只读这里）
_ask_menu_store = {}
_llm_menu_store = {}
_LLM_CALLBACK_PREFIX = "llm:"
_LLM_MENU_PROMPT = "请选择要切换到的 LLM："
...
async def _send_ask_user_menu(root_msg, event):
    ...
    # 候选为空: 纯问答场景, 直接发文本提示用户回复, 不构造空 markup (会被 telegram 拒)
    if not candidates:
        _BUS.pop_by_menu_id(menu_id)
        await root_msg.reply_text(event.question + "\n\n（请直接回复消息继续）")
        return
    multi = bool(event.multi) or bool(_MULTI_SELECT_RE.search(event.question or ""))
    ...
    try:
        while True:
            ...
            if done_item is not None:
                ...
                event = _BUS.drain_latest(_ASK_OWNER)
                if event:
...
async def handle_ask_callback(update, ctx):
    ...
    if ALLOWED and uid not in ALLOWED:
        _log_deny(update, "callback")
        return await query.answer(
            f"未授权。请联系管理员 {_DENY_ADMIN_URL} 申请加白 (你的 ID: {uid})",
            show_alert=True,
        )
    data = query.data or ""
    ...
    rest = parts[1:]
    if not menu_id or not rest:
        return await query.answer("菜单无效")
    stored = _ask_menu_store.get(menu_id)
    if not stored:
        await query.answer("菜单已过期")
        return await _clear_ask_reply_markup(query)
    question = stored.get("question") or "请选择下一步操作："
    candidates = list(stored.get("candidates") or [])
    multi = bool(stored.get("multi"))
    event_view = {"question": question, "candidates": candidates}
    action = rest[0]

    # multi: toggle 单项选中状态，原地刷新 markup
    if multi and action == _ASK_TOGGLE_ACTION and len(rest) >= 2:
        try:
            idx = int(rest[1])
        except ValueError:
            return await query.answer("菜单无效")
        if not (0 <= idx < len(candidates)):
            return await query.answer("菜单无效")
        selected = list(stored.get("selected") or [])
        if idx in selected:
            selected.remove(idx)
        else:
            selected.append(idx)
    ...
    # multi: done 提交所有勾选
    if multi and action == _ASK_DONE_ACTION:
        selected_indexes = list(stored.get("selected") or [])
        selected_labels = [candidates[i] for i in selected_indexes if 0 <= i < len(candidates)]
        if not selected_labels:
            return await query.answer("请至少选择一项")
        joined = "; ".join(selected_labels)
        _ask_menu_store.pop(menu_id, None)
        _BUS.pop_by_menu_id(menu_id)
        await query.answer()
        await _edit_ask_user_result(query, event_view, selected=joined)
    ...
    if action == _ASK_CANCEL_ACTION:
        ...
        _BUS.pop_by_menu_id(menu_id)
        await query.answer()
        await _edit_ask_user_result(query, event_view, cancelled=True)
    ...
    _BUS.pop_by_menu_id(menu_id)
    await query.answer()
    await _edit_ask_user_result(query, event_view, selected=selected)
    if query.message is None:
        return
...
async def _send_llm_menu(target_msg):
    ...
    if not llms:
        await target_msg.reply_text("⚠️ 当前没有可用 LLM")
        return
    menu_id = uuid.uuid4().hex
    _llm_menu_store[menu_id] = {"llms": llms}
...
async def handle_llm_callback(update, ctx):
    ...
    if not sep or not menu_id or not idx_str:
        return await query.answer("菜单无效")
    stored = _llm_menu_store.get(menu_id)
    if not stored:
        await query.answer("菜单已过期")
        try:
            await query.edit_message_reply_markup(reply_markup=None)
    ...
    try:
        await query.edit_message_text(
            f"✅ 已切换到 [{agent.llm_no}] {agent.get_llm_name()}",
            reply_markup=None,
        )
    except Exception as exc:
