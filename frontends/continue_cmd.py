...
def _snapshot_current_log(pid=None):
    ...
    snapshot = os.path.join(
        _LOG_DIR, f"model_responses_snapshot_{pid}_{stamp}_{time.time_ns() % 1_000_000_000:09d}.txt"
    )
...
_INJECT_MARKERS = ("### [WORKING MEMORY]", "[SYSTEM TIPS]", "[SYSTEM]", "[System]", "[DANGER]", "### [总结提炼经验]")
...
def _format_tool_use(block):
    """Match g_agent/loop.py:72 verbose tool-call header."""
    name = block.get("name", "?")
...
def _format_tool_result(content):
    """Match g_agent/loop.py:79-81 five-backtick fence around tool output."""
...
def _format_response_segment(response_body, tool_results):
    ...
    matching tool_result fences. Mirrors g_agent.loop verbose output so fold_turns
...
def extract_ui_messages(path):
    ...
    that g_agent.loop yields live, so fold_turns can fold them identically.
    """
    try:
    ...
    for i, (prompt, response) in enumerate(pairs):
        ...
        if user:
            ...
            # Turn 1 marker too — g_agent.loop yields one per LLM call, including the
...
def install(cls):
    ...
    if getattr(orig, "_continue_patched", False):
        return
