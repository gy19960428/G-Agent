...
os.environ.setdefault("G_AGENT_CHANNEL", "wechat")
...
class WxBotClient:
    ...
    def _load(self):
        if self._tf.exists():
            ...
            self.token, self.bot_id, self._buf = (
                d.get("bot_token", ""),
                d.get("ilink_bot_id", ""),
                d.get("updates_buf", ""),
            )
    ...
    def _save(self, **kw):
        d = {"bot_token": self.token or "", "ilink_bot_id": self.bot_id or "", "updates_buf": self._buf or "", **kw}
    ...
    def login_qr(self, poll_interval=2):
        r = requests.get(
            f"{API}/ilink/bot/get_bot_qrcode", params={"bot_type": 3}, headers={"User-Agent": UA}, timeout=10
        )
        r.raise_for_status()
        d = r.json()
    ...
    for item in items:
        for key, ext in _MEDIA_KEYS.items():
            ...
            try:
                ...
                ct = requests.get(
                    f"{CDN_BASE}/download?encrypted_query_param={quote(eq)}", headers={"User-Agent": UA}, timeout=60
                ).content
    ...
    def _handle():
        ...
        def _wx_send(text):
            ...
            try:
                ...
                print(
                    f"[WX] send err len={len(s)} dt={time.time()-t0:.1f}s {type(e).__name__}: {e}", file=sys.__stdout__
                )
                return False
        ...
        try:
            ...
            while True:
                ...
                if len(done) > sent:
                    merged = _clean("\n\n".join(done[sent:]))
                    print(
                        f"[WX] turns={len(done)}/{len(done)+1} sent={sent} sending={len(done)-sent}",
                        file=sys.__stdout__,
                    )
                    if _send(merged):
                        sent = len(done)
        ...
        for fpath in set(files):
            ...
            try:
                ...
                sender = (
                    ...
                    else bot.send_image if ext in {".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"} else bot.send_file
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.exit(1)
    _logf = open(
        os.path.join(os.path.dirname(os.path.dirname(__file__)), "temp", "wechatapp.log"),
        "a",
        encoding="utf-8",
        buffering=1,
    )
