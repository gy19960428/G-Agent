...
PLATFORMS = [
    {
        ...
        'desc': '直接用 python -m g_agent.agent 在终端交互',
        'deps': [],
    },
    {
        'id': 'telegram',
...
def probe_models(provider, apikey, apibase=None):
    ...
    if ptype == 'native_claude':
        ...
        headers = {'x-api-key': apikey, 'anthropic-version': '2023-06-01', 'User-Agent': 'G-Agent/1.0'}
    else:
        url = f"{base}/models"
        headers = {'Authorization': f'Bearer {apikey}', 'User-Agent': 'G-Agent/1.0'}
...
def generate_mykey(llm_cfgs, platform_configs):
    ...
    lines = []
    lines.append("# ══════════════════════════════════════════════════════════════════════════════")
    lines.append(f"#  G-Agent — mykey.py (由 configure.py 自动生成 @ {datetime.now().strftime('%Y-%m-%d %H:%M')})")
    lines.append("# ══════════════════════════════════════════════════════════════════════════════")
    lines.append("")
    lines.append("# ── 停止符 ──────────────────────────────────────────────────────────────────")
    ...
    # 尾部
    lines.append("# ══════════════════════════════════════════════════════════════════════════════")
    lines.append("#  配置完毕！运行: python -m g_agent.agent  (终端 REPL)")
    if platform_configs:
        for pc in platform_configs:
            p = pc['platform']
...
def main():
    ...
    print()
    if llm_cfgs:
        print(f"  {C['cyan']}  终端 REPL:{C['reset']}  python -m g_agent.agent")
    if platform_configs:
