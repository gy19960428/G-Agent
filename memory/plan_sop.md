...
⛔ **挂挡门禁（最易漏！）**：写出 plan.md ≠ 进入 plan 模式。**不调用 `enter_plan_mode` 就还是 40 turn 上限**，长任务必撞顶中断。
违规症状：plan.md 已写、按 step 推进、但 working memory 里没有 `in_plan_mode` 字段。

**两步合一，必须连续执行**：
1. 创建工作目录 `./plan_XXX/`（XXX=任务英文短名）并写 `plan.md`
2. **紧接着**用一个 code_run 调用 `handler.enter_plan_mode("./plan_XXX/plan.md")` 挂挡（max_turns 40→100）：
   `code_run({'inline_eval':True, 'script':'handler.enter_plan_mode("./plan_XXX/plan.md")'})`

注：g_agent.py 已加自动挂挡兜底——通过 `file_write`/`file_patch` 写入 `plan_*/plan.md` 时自动进入 plan 模式。但若用 `code_run` 间接写盘则**不会触发兜底**，仍需手动挂挡。
