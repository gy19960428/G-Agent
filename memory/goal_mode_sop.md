...
start /b python -m g_agent.agent --reflect reflect/goal_mode.py

# 自定义路径（多实例）
set GOAL_STATE=temp/goal_xxx.json && start /b python -m g_agent.agent --reflect reflect/goal_mode.py

# 用其他模型跑（--llm_no 选择已配置的第N个LLM，从0开始）
set GOAL_STATE=temp/goal_xxx.json && start /b python -m g_agent.agent --reflect reflect/goal_mode.py --llm_no 1
```
